class tcp
{
  ...
  accept(host="", port=0)
  {
    if ((s := this.acceptClient(host, port))!=0)
    {
...
UdpSend( message ) {
  myUdpOut := new udp()
  myUdpOut.connect("addr_broadcast", 7999)
  myUdpOut.enableBroadcast()
  myUdpOut.send( message )
}

UdpSendPort(message, port) {
  myUdpOut := new udp()
  myUdpOut.connect("addr_broadcast", port)
  myUdpOut.enableBroadcast()
  myUdpOut.send( message )
}